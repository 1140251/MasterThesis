...
class Oyente(Parser):
    ...
    def extract_result_line(self, line):
        ...
        key = line[:index_split].lower().replace(
    ...
    def parse(self, str_output):
        ...
        for line in lines:
            if "INFO:root:contract" in line:
                ...
                (file, contract_name, _) = line.replace(
                    "INFO:root:contract ", '').split(':')
                ...
                (line, column, level, message) = line.replace(
                    "INFO:symExec:%s:" % (current_contract['file']), '').split(':')
